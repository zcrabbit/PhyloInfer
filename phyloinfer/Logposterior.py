...
def GradLogpost(tree, branch, D, U, beta, pden, L, scale=0.1, surrogate=False, delta = 0.01):
    if surrogate:
        maped_branch = [mollifier(blen,delta) for blen in branch]
        return (-phyloLoglikelihood(tree, maped_branch, D, U, beta, pden, L, grad=True)  
                - phyloLogprior_exp(maped_branch, scale, grad=True)) * np.minimum(branch,delta)/delta
    else:    
        return -phyloLoglikelihood(tree, branch, D, U, beta, pden, L, grad=True) - phyloLogprior_exp(branch, scale, grad=True)